...
class _ReportDetailsPageState extends State<ReportDetailsPage>
  ...
  // ── Shorthand localizations accessor ─────────────────────────────────────
  AppLocalizations get l10n => AppLocalizations.of(context)!;

  // ── Text helpers ──────────────────────────────────────────────────────────
  String safe(dynamic v) => v?.toString().trim() ?? 'N/A';

  String loc(dynamic field, {String fallback = 'N/A'}) =>
      ReportsService.extractText(field, lang: _currentLang, fallback: fallback);
  ...
  String get _categoryName {
    final v = widget.emergency['categoryName'];
    if (v is String && v.trim().isNotEmpty) return v;
    return loc(widget.emergency['category']?['name']);
  }

  String get _typeName {
    final v = widget.emergency['typeName'];
    if (v is String && v.trim().isNotEmpty) return v;
    return loc(widget.emergency['emergencyType']?['name']);
  }

  String get _description => loc(widget.emergency['description']);

  String get _kebeleName {
    final k = widget.emergency['kebele'] ?? widget.emergency['Kebele'];
    if (k == null) return 'N/A';
    if (k is Map) return loc(k['name']);
    return loc(k);
  }
      ...
      child: Scaffold(
        ...
        body: FadeTransition(
          ...
          child: CustomScrollView(
            ...
            slivers: [
              SliverToBoxAdapter(
                child: _buildHeader(context, e, status, sColor, sIcon),
              ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Description
                      // ✅ was: _sectionLabel("DESCRIPTION")
                      _sectionLabel(l10n.rdSectionDescription),
                      ...
                      // Location card
                      // ✅ was: _sectionLabel("LOCATION")
                      _sectionLabel(l10n.rdSectionLocation),
                      const SizedBox(height: 12),
                      _locationCard(),
                      const SizedBox(height: 28),
  ...
  Widget _buildHeader(BuildContext context, Map<String, dynamic> e,
      String status, Color sColor, IconData sIcon) {
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ✅ was: "Emergency Report" (hardcoded)
                  Text(
  ...
  Widget _buildHeroImage(String? mediaUrl) {
    ...
    if (mediaUrl != null && mediaUrl.isNotEmpty) {
      final url = mediaUrl.startsWith('http')
          ? mediaUrl
          : ReportsService.getFullImageUrl(mediaUrl);
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // ✅ was: "BAHIR DAR · KEBELE" (hardcoded)
              Text(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          _field(l10n.rdFieldKebele, kebeleC),
          _field(l10n.rdFieldDescription, descC, maxLines: 4),
        ]),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
